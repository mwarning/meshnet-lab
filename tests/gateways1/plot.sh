...
for id in 'line' 'grid4' 'rtree'; do
	gnuplot -e "
		...
		'${prefix}gateways1-yggdrasil-0.3.16-$id.csv' using (column('node_count')):(100 * column('packets_received') / column('packets_send')) with points linetype rgb 'dark-violet' title 'yggdrasil-0.3.16 [%]' axis x1y2, \
		\
		...
		'${prefix}gateways1-yggdrasil-0.4.7-$id.csv' using (column('node_count')):(100 * column('packets_received') / column('packets_send')) with points linetype rgb 'skyblue' title 'yggdrasil-0.4.7 [%]' axis x1y2, \
		\
		...
		'${prefix}gateways1-yggdrasil-0.5.5-$id.csv' using (column('node_count')):(100 * column('packets_received') / column('packets_send')) with points linetype rgb 'dark-yellow' title 'yggdrasil-0.5.5 [%]' axis x1y2, \
		;\
	"
done
