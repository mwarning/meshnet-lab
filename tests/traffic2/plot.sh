...
gnuplot -e "
	...
	set ytics nomirror;																	\
	plot																				\
	'traffic2-batman-adv-lattice4_new.csv' using (column('node_count')):(column('rx_bytes') / 1000 / column('duration_ms') / 1000 / column('node_count')):(column('rx_bytes_range') / 1000 / column('duration_ms') / 1000 / 2) with errorbars title '' axis x1y1,	\
	'traffic2-batman-adv-lattice4_new.csv' using (column('node_count')):(column('rx_bytes') / 1000 / column('duration_ms') / 1000 / column('node_count')) with linespoints title 'rx per node [KB/s]' axis x1y1,					\
